...
import { getPaymentHistories } from 'apiSdk/payment-histories';
...
function DashboardPage() {
  ...
  const paymentHistoryStat = useSWR<PaginatedInterface<PaymentHistoryInterface>>(
    () => `/payment-histories`,
    () => getPaymentHistories({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: paymentHistoryStat?.data?.totalCount,
      isLoading: paymentHistoryStat.isLoading,
