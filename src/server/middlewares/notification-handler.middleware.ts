...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'client.update': { roles: ['owner'], key: 'client-info-updated', tenantPath: ['client'], userPath: [] },
  'payment_history.create': {
    roles: ['owner'],
    key: 'new-payment-recorded',
    tenantPath: ['client', 'payment_history'],
    userPath: [],
  },
  'invoice.create': { roles: ['owner'], key: 'invoice-generated', tenantPath: ['client', 'invoice'], userPath: [] },
};

const ownerRoles: string[] = ['owner'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['owner', 'accountant'];
